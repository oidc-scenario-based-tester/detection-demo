...
app.post('/login', async (req, res) => {
    ...
    res.redirect(`/consent?${new URLSearchParams(req.session.authDetails).toString()}`); // Redirect to the consent screen after successful login
});
...
app.post('/consent', (req, res) => {
  ...
  const code = uuidv4();
  codes[code] = { client_id, redirect_uri, state, scope, userId: req.session.userId };
