...
function generateIdToken(userId, clientId, nonce) {
...
app.get('/authorize', (req, res) => {
  const { response_type, client_id, redirect_uri, state, scope, nonce } = req.query;
  
  const client = clients[client_id];
  if (!client || !redirect_uri || !state || !scope  || !nonce || !client.redirectUris.includes(redirect_uri)) {
  ...
  req.session.authDetails = { client_id, redirect_uri, state, scope, response_type, nonce };
  res.redirect('/login');
});
...
app.post('/consent', (req, res) => {
    ...
    const { client_id, redirect_uri, state, nonce, response_type } = authDetails;
    const userId = req.session.userId;

    if (response_type === 'id_token') {
      // If the response type is id_token, generate the ID Token and redirect back to the client
      const idToken = generateIdToken(userId, client_id, nonce);
      const redirectUrl = new URL(redirect_uri);
      redirectUrl.hash = `id_token=${idToken}&state=${state}`; // ID Token is included in the URL fragment
      res.redirect(redirectUrl.toString());
    } else {
      // Handle the response for other response types (like 'code')
      const code = uuidv4();
      codes[code] = { ...authDetails, userId: userId };
